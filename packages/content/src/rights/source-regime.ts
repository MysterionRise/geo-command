...
const FALLBACK_SOURCES: readonly EvidenceSourceClass[] = Object.freeze([
  "project-owned-human",
  "model-output",
]);

const STACK_OVERFLOW_SOURCES: readonly EvidenceSourceClass[] = Object.freeze([
  "model-output",
  "stack-overflow",
]);
...
const freezeDetermination = (
  ...
  const consideredLicenseVersions = stringList(
    input.consideredLicenseVersions,
  ...
  const coveredItems = Object.freeze(input.coveredItems.map((covered, index) => {
    const item = Object.freeze({
      ...
      licenseVersion: nonBlank(
        covered.licenseVersion,
        `coveredItems[${index}].licenseVersion`,
      ),
    });
  ...
  return Object.freeze({
    ...
    contributionRevisionDateTreatment: nonBlank(
      input.contributionRevisionDateTreatment,
    ...
    presentationDesignVersion: nonBlank(
      input.presentationDesignVersion,
      "presentationDesignVersion",
    ),
    interactionDesignVersion: nonBlank(
      input.interactionDesignVersion,
